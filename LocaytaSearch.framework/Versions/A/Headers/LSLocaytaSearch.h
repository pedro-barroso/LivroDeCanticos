...
\endcode
 ...
 \section field_options Field Options
  ...
  below. More options may be added over time as functionality is added to the search engine.
   ...
   - textslot (integer) : slot number to store values in for text (case-insensitive) sorting. See below for details about slot numbers.

   - stringslot (integer) : slot number to store values in for string literal text facet collection. See below for details about slot numbers.
 
   - numericslot (integer) : slot number to store values in for numeric sorting and numeric (ranges) facet collection. See below for details about slot numbers.
  
  Slot numbers will often map one-to-one on field names, but occasionally there are circumstances where one field will need more than one slot. Slot numbers are shared between textslots, stringslots, and numericslots, and using the same slot number for more than one will lead to undefined behaviour.
...
\endcode
  ...
  The list of results is provided in the \ref LSLocaytaSearchResult::results property as an NSDictionary; see the API documentation for its format.
...
\endcode
 ...
 \page facets Generating facets
 
 Faceted search, also called faceted navigation or faceted browsing, allows users to
 narrow their results by selecting one or several options to filter the results by.
 
 Classically this is done by using a hard-coded list of options for any facet,
 for instance a 'colour' facet might be pre-populated with every known colour
 used by the data.
 
 Locayta Search Mobile adds the ability to dynamically generate the list of facet
 options based on the current search results, so that users aren't presented with
 options that would yield no results. This also allows interface designers to present
 the number of items matching each facet option to the user.
 
 \section setup Setting up an index to enable facet generation
 
 You will first need to set up your index appropriately to filter the search
 results by the options generated, if you have not already done so.
 This is usually straightforward:
 
 - For text-based filters, for instance 'colour' or 'genre', index the field
   using the 'boolean' \ref schema option.
 
 - For numeric filters that you want to filter by a range of values, for instance
   'price', index the field using the 'numericslot' \ref schema option.
 
 Locayta Search Mobile is able to use data stored using both the 'boolean' and
 various slot schema options to generate facets. Whether you store the data
 for facets in boolean data or slot data will largely depend on your needs.
 
  - Boolean data is able to store more than one value per field per document.
    For example, if you are storing a colour field, each of your documents could
    contain more than one colour.
 
  - Slot data is only able to store one value per field per document, but tends
    to be faster to generate facets for than boolean data.

 As a general rule of thumb, use boolean data for text fields, and slot data
 for numeric fields, unless you have any particular reason not to. This cuts down
 on data duplication from storing both boolean and slot data for each document.
 
 \section howitworks How it works
 
 When you perform a search, you will give Locayta Search Mobile a list of the
 boolean fields and slot numbers that you want to generate facets for. While
 Locayta is performing its main match routine (acquiring matching documents and
 calculating the weighting of each one for relevance sorting), it takes time out
 to inspect the boolean or slot data for each matching document, tallying up each separate
 option it finds. These tallies are handed back to the user in the results object.
 ...
 // Determine which boolean facet options or other filters need to be applied (if any):
 NSDictionary *filters = [NSDictionary dictionaryWithObjectsAndKeys:
                          [NSArray arrayWithObject:@"red"], @"colour",
                          nil];
 query.filters = filters;
 
 // Determine which numeric ranges have been selected (if any):
 [query addNumericRangeFrom:0 to:10 forSlot:1];
 
 // Initialise the request object (this would usually be done elsewhere, eg in -(id)init):
 LSLocaytaSearchRequest *request = [[LSLocaytaSearchRequest alloc] initWithDatabasePath:dbpath delegate:self]
 
 // Perform the search, requesting all relevant facets to be generated:
 NSArray *facets = [NSArray arrayWithObjects:
                    @"colour", @"genre",        // boolean facets
                    [NSNumber numberWithInt:1], // slot facets
                    nil];
 [request searchWithQuery:query topDocIndex:0 docsPerPage:10 facets:facets];
 
 \endcode
 
 Parsing the facet data in the returned results:
 
 \code 
 - (void) locaytaSearchRequest:(LSLocaytaSearchRequest *)searchRequest didCompleteWithResult:(LSLocaytaSearchResult *)searchResult {
    NSDictionary *facets = searchResult.facets;
    for (NSObject *key in [facets allKeys]) {
        NSString * fieldName;
        if ([key isKindOfClass:[NSNumber class]]) {
            // slot facet
            fieldName = [someSlotLookup objectForKey:key]:
        } else {
            // boolean facet
            fieldName = [key copy]; // boolean names are usually the same as field names
        }
        NSLog(@"Options for %@:", fieldName);
        
        for (NSObject *option in [[facets objectForKey:key] allKeys]) {
            // option will either be an NSNumber or NSString depending on
            // whether the slot was a numericslot or other kind of slot.
            // Boolean facet options are always NSString.
 
            // Dictionary values will be NSNumber objects containing an
            // integer value for the count of the number of times each option
            // appeared in the results.
            NSLog(@" - %@ (%@ results)", option, [[facets objectForKey:key] objectForKey:option]);
        }
    }
 }
 \endcode
 
 You can also refer to LSLocaytaSearchResult::facets for an example of what
 the facets dictionary looks like.
 
 The format for facets that contain numeric data is probably not what an
 end user would expect in most situations. For instance, if the facet was "price",
 they probably don't want to see a list like this:
 ...
 It is more likely they expect to see a list such as:
 
  - $0 - $10 (120 results)
  - $10 - $20 (35 results)
  - ... etc.
 
 In this case, you can use the class methods provided by LSLocaytaSearchResult
 to transform the facet data into something more user-friendly.
 LSLocaytaSearchResult#convertNumericFacetData:toRanges: and
 LSLocaytaSearchResult#convertNumericFacetData:toDistributedRanges: provide
 automatic grouping, while LSLocaytaSearchResult#convertNumericFacetData:toFixedRanges:
 provides manual grouping.
 ...
 \page glossary Glossary
  ...
  This list provides definitions for common terms used in Locayta Search Mobile.
   ...
   - <b>Balancer</b>
      - A balancer is a class that can be used to alter the order of search results
        in a more sophisticated way than simple sorting by text relevance or a
        field value. Balancers allow the order of results to be "balanced" between
        text relevane and another measurement, for instance the value stored in
        a field or the geographic distance of each result from the user's location.\n
        \n
        The amount each balancer affects the order of results is given by its
        \ref balance_factors "balance factor".\n
        \n
        This is also known as boosting, biasing, or mixing the results.
...
#import "LSLocaytaSearchDistanceBalancer.h"
#import "LSLocaytaSearchStoredValBalancer.h"
