 ...
 *
 * There should only be one LSLocaytaSearchIndexer per index at any given time
 * to avoid multiple processes simultaneously writing to the file, which could
 * result in a corrupted index being generated.
 */
 ...
 *	an error if it was not passed in as NULL.
 */
 ...
 *
 * @param aDatabasePath An existing search database directory. Use
 *        \ref createDatabaseAtPath:error: to create the database first.
 *
 * @param aDelegate An object implementing the LSLocaytaSearchIndexerDelegate
 *        protocol. It can be nil if no callbacks are required.
 */
 ...
 *
 * @param indexableRecord The item to delete. Only needs to contain the "id"
 *        field of the record to delete.
 *
 * The operation will be performed asynchronously. The delegate will be notified
 * when complete.
 */
 ...
 *
 * @param indexableRecords An array of LSLocaytaSearchIndexableRecord objects.
 *
